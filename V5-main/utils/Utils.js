...
class ManaDetector {
    constructor() {
        ...
        this.lastNotEnoughManaAt = 0;
        this.notEnoughManaPattern = /not enough mana/i;
    ...
    onGameMessage(packet) {
        ...
        if (this.notEnoughManaPattern.test(stripped)) {
            this.currentMana = 0;
            this.lastNotEnoughManaAt = Date.now();
            return;
        }
    ...
    isRecentlyOutOfMana(windowMs = 1500) {
        return this.lastNotEnoughManaAt > 0 && Date.now() - this.lastNotEnoughManaAt <= windowMs;
    }
...
class UtilsClass {
    ...
    isRecentlyOutOfMana(windowMs) {
        return manaDetector.isRecentlyOutOfMana(windowMs);
    }
