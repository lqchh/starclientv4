...
class SwiftIntegration {
    constructor() {
        ...
        this.DEFAULT_MAX_COMPUTE = 500000;
        this.MIN_MAX_COMPUTE = 1;
        this.MAX_MAX_COMPUTE = 5000000;
    }
    ...
    toJavaPointArray(points, isFly) {
        if (!this.isPointList(points)) return null;
    ...
    SwiftPath(startPoints, endPoints, isFly = false, variantSeed = 0, maxCompute = 500000) {
        ...
        const computeLimit = this.normalizeMaxCompute(maxCompute);
        const startsValid = this.isPointList(startPoints);
        const endsValid = this.isPointList(endPoints);
    ...
    isPointList(points) {
        return Array.isArray(points) && points.length > 0 && points.every((point) => Array.isArray(point) && point.length >= 3);
    }

    normalizeMaxCompute(value) {
        const parsed = Math.floor(Number(value));
        if (!Number.isFinite(parsed)) return this.DEFAULT_MAX_COMPUTE;
        return Math.max(this.MIN_MAX_COMPUTE, Math.min(this.MAX_MAX_COMPUTE, parsed));
    }
    ...
    getResult() {
        ...
        const result = {
            ...
            path_cost: this.getPathCost(),
            selected_start_index: this.getSelectedStartIndex(),
            selected_goal_index: this.getSelectedGoalIndex(),
    ...
    getSelectedGoalIndex() {
        const index = PathManager.getSelectedGoalIndex();
        return typeof index === 'number' ? index : -1;
    }

    getPathCost() {
        const cost = PathManager.getPathCost();
