...
class PathRotsUtil {
    constructor() {
        ...
        this.tickRegister = register('step', () => this.tick()).setFps(150).unregister();
    ...
    applyRotationWithGCD(yaw, pitch) {
        ...
        RotationGCD.applyToPlayer(yaw, Math.max(-90, Math.min(90, pitch)));
    }
    ...
    rotateToAngles(yaw, pitch, instant = false, durationMs = 500) {
        ...
        this.targetYaw = yaw;
        this.targetPitch = Math.max(-90, Math.min(90, pitch));
        this.targetVector = null;
        this.instantMode = instant || !Number.isFinite(durationMs) || durationMs <= 0;
        this.rotating = true;
        ...
        this.pitchDiff = this.targetPitch - this.initialPitch;
