...
class Finder {
        ...
        this.tick = register('tick', () => {
            ...
            if (!result || !result.keynodes?.length) {
                if (this.checkIfReachedDestination()) {
                    ...
                    this.finishFailure('No path found' + (reason ? ': ' + reason : ''));
                }
                return;
            }
            ...
            if (!this.saidInfo && this.calledFromFile && PathConfig.PATHFINDING_DEBUG) {
                ...
                const pathCost = Number(result.path_cost);
                if (Number.isFinite(pathCost) && pathCost > 0) {
                    Chat.messagePathfinder(`Path cost: ${Math.round(pathCost * 10) / 10}`);
                }
                this.saidInfo = true;
            }
            ...
            if (!this.isFly) {
                ...
                if (!splinePath?.length) {
                    this.finishFailure('Pathfinder returned no walk spline.');
                    return;
                }
                ...
                if (Rotations.failed) {
                    this.recalculate('rotation_lookahead_failed');
                    return;
                }
                ...
                if (this.flyStarted && this.flyStartDelayTicks === 0) {
                    ...
                    if (FlyMovement.isActive === false) {
                        ...
                        this.finishFailure('Fly path stopped before reaching the destination.');
                        return;
                    }
                }
        ...
        switch (action) {
            ...
            case 'BACKUP_RECALC':
                ...
                Movement.backup(15, () => {
                    ...
                    if (this.pathSettled) return;
                    this.recalculate('backup_recalc');
                });
                break;
        }
    }
    ...
    recalculate(reason = 'generic') {
        ...
        if (this.recalculateAttempts > this.MAX_RECALCULATE_ATTEMPTS) {
            ...
            this.finishFailure('Max recalculation attempts, failed!');
            return;
        }
        ...
        ScheduleTask(3, () => {
            ...
            this.findPath(end, callback, wasFly, starts, true);
        });
    }
    ...
    retryRecalculate() {
        this.pathVariantSeed = Math.max(this.pathVariantSeed, this.recalculateAttempts);
        if (!this.isFly && this.samePathSignatureCount >= 2) {
            this.pathVariantSeed += 1;
    ...
    finishFailure(reason = 'Pathfinding failed.', notify = false) {
        this.finishPath(false, reason, notify);
    }

    finishPath(success, reason = null, notify = false) {
        if (this.pathSettled) return;
        this.pathSettled = true;
        const callback = this.currentCallback;

        if (success) {
            showNotification('Path Complete', 'Destination reached!', 'SUCCESS', 2000);
        } else {
            const message = reason || 'Pathfinding failed.';
            Chat.messagePathfinder('§c' + message);
            if (notify) showNotification('Pathfinding Failed', message, 'ERROR', 5000);
        }

        this.resetPath();
        PathExecutor.destroy();
        this.callCallback(!!success, callback);
    }

    callCallback(success, callback = this.currentCallback) {
    ...
    failWarpPathfinding() {
        ...
        this.finishFailure(`Failed to warp to ${warpName} after ${this.MAX_WARP_RETRIES} retries.`, true);
    }
