...
// Hypixel uses "Area:" in most places and "Dungeon:" while inside Catacombs (see Devonian Location.kt).
const SKYBLOCK_AREA_REGEX = /^(?:Area|Dungeon):\s*([\w ']+)$/i;
...
class TabListUtilsClass {
    ...
    formatTabLine(entry) {
        try {
            if (entry && typeof entry.getName === 'function') {
                const name = entry.getName();
                if (name && typeof name.getUnformattedText === 'function') return this.stripFormatting(name.getUnformattedText());
                return this.stripFormatting(String(name));
            }
        } catch (e) {}

        return this.stripFormatting(String(entry ?? ''));
    }
    ...
    getArea() {
        ...
        try {
            ...
            for (let i = 0; i < tabLines.length; i++) {
                const cleanLine = this.formatTabLine(tabLines[i]).trim();
                const match = cleanLine.match(SKYBLOCK_AREA_REGEX);
                if (!match) continue;

                const detectedArea = match[1].trim();
